...
groundhog.library(c("dplyr", "tidyr"), groundhogDay)

# Set column names for output from file
ColumnNames <- c("CommissioningOrganisation", "Organisation", "OrganisationCode", "60to69_eligible-on-last-day", 
...
test2 <- test %>%
  ...
  select(1, 17, everything()) %>% #reorders the data frame to move the new column to the organisation columns
  fill(names(test2[c(1,2)]), .direction = "down") %>% # fill the data for the region and commissioning organisation down 
  filter(!is.na(Organisation)) %>% # remove the rows where there is no Organisation (Removes the blank region rows)
  filter(CommissioningOrganisation != Organisation) %>% #Remove the rows for the commissioning organisation (can aggregate up from GP data)
  select(-17) #drop the empty row at the end

ColNamesAll <- c(names(test2[1]), ColumnNames) #create a list of column names, keeping the first column name

names(test2) <- ColNamesAll # rename the columns based on the list
